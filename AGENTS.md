...
- Все Remote Config ключи читаются только из выбранного paywall `main`;
  продукты, variation и SDK references остаются у своего placement. Запрет
  из `main` не теряется при ошибке продуктов и не заменяется чужим config.
...
- Standard Special Offer читает strict boolean gate из выбранного
  paywall плейсмента `main`, загружает все products из отдельного placement и не
