...
- Сохранённые active/expired/cooldown состояния переносятся с `special-offer`
  на `special_offer` без изменения дат. Новый ключ имеет приоритет; ошибка
  хранения не открывает новое окно. Reset сохраняет отметку, запрещающую
  повторный импорт старого цикла после перезапуска. Добавлен migration probe.
...
хранится по raw ID; миграция сохраняет текущий цикл при переходе на новый ID.
