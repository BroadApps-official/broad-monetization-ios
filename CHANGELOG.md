...
- `ProductPricePresenter.presentation(for:among:)` — презентация одного продукта
  внутри его набора, без сопоставления результата по `presentationID`.
...
Одиночного аксессора не было, поэтому строка пейвола на каждой перерисовке
считала презентации всего набора и искала свою. Веса месяца и года были
недоступны, и приложения объявляли `Decimal(string: "4.348")!` у себя, чтобы
отсортировать тарифы по длине.
